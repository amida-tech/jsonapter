...
function evaluateExistsFunction (existsObj, input) {
    return _.isFunction(existsObj) ? existsObj(input) : _.iteratee(existsObj)(input);
}
...
var prototype = {
    ...
    template: function (template, input, params) {
        var templateValue = template.template;
        return this.evaluateValue(templateValue, input, params);
    },
    runForArray: function (template, input, params) {
        ...
        var result = input.reduce(function (r, e) {
            var value = that.run(modifiedTemplate, e, params);
            if (value !== null) {
                r.push(value);
            }
            return r;
    ...
    run: function (template, input, params) {
        ...
        if (Array.isArray(input)) {
            if (template.single) {
                ...
                var resultForArray = this.runForArray(template, input, params);
